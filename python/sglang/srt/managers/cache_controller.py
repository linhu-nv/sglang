...
# which implements the kv.fetch action). Kept as a literal here so this
...
class HiCacheController:
    def __init__(
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            self.storage_lifecycle_sync_groups = self._create_lifecycle_sync_groups()
    ...
    def detach_storage_backend(self, coordinated_lifecycle: bool = False):
        ...
        if not self._storage_lifecycle_vote(stop_error is None, coordinated_lifecycle):
        ...
        if not self._storage_lifecycle_vote(close_error is None, coordinated_lifecycle):
            self._mark_storage_lifecycle_failed()
