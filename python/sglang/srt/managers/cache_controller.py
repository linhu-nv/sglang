...
# Free-form key under HiCacheStorageExtraInfo.extra_info that carries the
# per-request KV-hint envelope down to storage backends (currently read by the
# KVCR backend, which implements the kv.fetch action). Kept as a literal here so
# this generic controller stays backend-agnostic; the KVCR backend owns the
# matching constant, and a test pins the two together.
_KV_HINTS_EXTRA_INFO_KEY = "kv_hints"


def _kv_hints_extra_info(operation: StorageOperation) -> Optional[dict]:
    """Wrap an operation's KV-hint envelope into extra_info, or None if absent."""
    if operation.kv_hints is None:
        return None
    return {_KV_HINTS_EXTRA_INFO_KEY: operation.kv_hints}
...
class StorageOperation:
    ...
    def __init__(
        ...
        # Per-request KV-hint envelope, injected into HiCacheStorageExtraInfo so
        # a storage backend (KVCR) can locate the source of a remote prefix.
